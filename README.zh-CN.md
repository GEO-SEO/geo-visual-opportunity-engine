...
# GEO Visual Content Engine 中文说明

> 把 GEO 机会、产品信息和关键词，变成 AI 生成视觉素材、本地化内容和电商可用资产。

## 它解决什么问题

- 电商团队经常把研究、图片生成、产品数据、本地化文案和平台导出拆在不同工具里做。
- 这个项目把这些步骤合成一个面向 GEO 和 AI-native discovery 的视觉内容生产流程。

## 什么时候用它

- 你有产品和关键词机会，需要快速生成视觉和内容资产。
- 你面向 Shopify、WooCommerce 或类似电商平台。
- 你希望输出不仅是图片，还有本地化内容和结构化产品数据。

## 和相邻项目有什么区别

- 它偏“视觉和电商资产”。
- `seo-geo-content-engine` 偏长文和 SEO/GEO 内容。
- `brand-ai-performance-check` 偏品牌表现报告。
...
1. 准备产品输入、目标关键词、品牌要求和平台目标。
2. 运行视觉内容工作流。
3. 检查图片、文案和结构化数据后导出到电商平台。
